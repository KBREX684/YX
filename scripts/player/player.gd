...
## PlayerController —— P1 微切片玩家控制器。
##
## 只读取 Input Map action；噪声通过 EventBus 广播给后续怪物系统。
...
func _physics_process(delta: float) -> void:
	...
	apply_movement_intent(
		input_vector,
		Input.is_action_pressed(ACTION_RUN),
		Input.is_action_pressed(ACTION_CROUCH),
		_dominant_action(input_vector),
		delta
	)
	move_and_slide()
...
func apply_movement_intent(
	input_vector: Vector2,
	is_running: bool,
	is_crouching: bool,
	source_action_id: StringName,
	delta: float = 0.016
) -> void:
...
func tick_flashlight(delta: float) -> void:
	if not _flashlight_enabled:
		_sync_flashlight_visual()
		return
	_flashlight_battery = maxf(
		0.0,
		_flashlight_battery - _flashlight_float(&"battery_drain_per_second") * delta
	)
...
func _set_movement_state(next_state: StringName, source_action_id: StringName) -> void:
	...
	var level: int = STATE_NOISE.get(next_state, 0)
	EventBus.noise_emitted.emit(level, global_position, source_action_id)
	print("EventBus.noise_emitted level=%d action=%s" % [level, source_action_id])
